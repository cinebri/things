...
async function updateWallet(wallet, category) {
  try {
    ...
    if (walletElement) {
      ...
      walletElement.querySelector(
        ".terrahash"
      ).textContent = `Hashes: ${wallet.terrahash}`;
      walletLoadStatus[category][wallet.url] = true;
      updateMinedTokens();
      return true;
    } else {
...
async function updateBurnedTokens() {
  try {
    ...
    const data = await response.json();
    let burnedTokens = Math.floor(
      500000000 - parseFloat(data.total_supply) / 1000000000
    );
  ...
  try {
    ...
    for (const category in walletLoadStatus) {
      for (const wallet in walletLoadStatus[category]) {
        if (walletLoadStatus[category][wallet]) {
          loadedwallets += 1;
        }
      }
    }
...
async function updateWallets() {
  ...
  for (const category in wallets) {
    for (const wallet of wallets[category]) {
      totalwallets += 1; //всего кошельков
    }
  }
  try {
    for (const category in wallets) {
      for (const wallet of wallets[category]) {
        ...
        // const success = updateWallet(wallet, category, totalwallets);
        // if(success){
